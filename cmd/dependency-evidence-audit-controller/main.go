// Command dependency-evidence-audit-controller runs the evidence-audit lane
// controller: the read-only proof surface of the internal evidence index.
package main
...
var (
	exitProcess = os.Exit
	run         = bootstrap.RunEvidenceAudit
	lookupEnv   = os.Getenv
...
func runMain(ctx context.Context, arguments []string, lookup func(string) string, build bootstrap.PortsBuilder, stdout io.Writer, stderr io.Writer) int {
	if len(arguments) == 1 && arguments[0] == "--version" {
		fmt.Fprintf(stdout, "dependency-evidence-audit-controller %s\n", version)
		return 0
	}
