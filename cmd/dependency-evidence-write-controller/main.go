// Command dependency-evidence-write-controller runs the evidence-write lane
// controller: the only writer of the operations-evidence class.
package main
...
var (
	exitProcess = os.Exit
	run         = bootstrap.RunEvidenceWrite
	lookupEnv   = os.Getenv
...
func runMain(ctx context.Context, arguments []string, lookup func(string) string, build bootstrap.PortsBuilder, stdout io.Writer, stderr io.Writer) int {
	if len(arguments) == 1 && arguments[0] == "--version" {
		fmt.Fprintf(stdout, "dependency-evidence-write-controller %s\n", version)
		return 0
	}
